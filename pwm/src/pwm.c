...
static int en7523_pwm_probe(struct platform_device *pdev)
{
	...
	pc->flash_mode_cfg = devm_platform_ioremap_resource(pdev, 0);
	if (IS_ERR(pc->flash_mode_cfg))
		return PTR_ERR(pc->flash_mode_cfg);

	pc->flash_prd_set[0] = devm_platform_ioremap_resource(pdev, 1);
	if (IS_ERR(pc->flash_prd_set[0]))
		return PTR_ERR(pc->flash_prd_set[0]);

	pc->flash_prd_set[1] = devm_platform_ioremap_resource(pdev, 2);
	if (IS_ERR(pc->flash_prd_set[1]))
		return PTR_ERR(pc->flash_prd_set[1]);

	pc->flash_prd_set[2] = devm_platform_ioremap_resource(pdev, 3);
	if (IS_ERR(pc->flash_prd_set[2]))
		return PTR_ERR(pc->flash_prd_set[2]);

	pc->flash_prd_set[3] = devm_platform_ioremap_resource(pdev, 4);
	if (IS_ERR(pc->flash_prd_set[3]))
		return PTR_ERR(pc->flash_prd_set[3]);

	pc->flash_map_cfg[0] = devm_platform_ioremap_resource(pdev, 5);
	if (IS_ERR(pc->flash_map_cfg[0]))
		return PTR_ERR(pc->flash_map_cfg[0]);

	pc->flash_map_cfg[1] = devm_platform_ioremap_resource(pdev, 6);
	if (IS_ERR(pc->flash_map_cfg[1]))
		return PTR_ERR(pc->flash_map_cfg[1]);

	pc->cycle_cfg_value[0] = devm_platform_ioremap_resource(pdev, 7);
	if (IS_ERR(pc->cycle_cfg_value[0]))
		return PTR_ERR(pc->cycle_cfg_value[0]);

	pc->cycle_cfg_value[1] = devm_platform_ioremap_resource(pdev, 8);
	if (IS_ERR(pc->cycle_cfg_value[1]))
		return PTR_ERR(pc->cycle_cfg_value[1]);

	pc->sipo.flash_mode_cfg = devm_platform_ioremap_resource(pdev, 9);
	if (IS_ERR(pc->sipo.flash_mode_cfg))
		return PTR_ERR(pc->sipo.flash_mode_cfg);

	pc->sipo.flash_map_cfg[0] = devm_platform_ioremap_resource(pdev, 10);
	if (IS_ERR(pc->sipo.flash_map_cfg[0]))
		return PTR_ERR(pc->sipo.flash_map_cfg[0]);

	pc->sipo.flash_map_cfg[1] = devm_platform_ioremap_resource(pdev, 11);
	if (IS_ERR(pc->sipo.flash_map_cfg[1]))
		return PTR_ERR(pc->sipo.flash_map_cfg[1]);

	pc->sipo.flash_map_cfg[2] = devm_platform_ioremap_resource(pdev, 12);
	if (IS_ERR(pc->sipo.flash_map_cfg[2]))
		return PTR_ERR(pc->sipo.flash_map_cfg[2]);

	pc->sipo.led_data = devm_platform_ioremap_resource(pdev, 13);
	if (IS_ERR(pc->sipo.led_data))
		return PTR_ERR(pc->sipo.led_data);

	pc->sipo.clk_divr = devm_platform_ioremap_resource(pdev, 14);
	if (IS_ERR(pc->sipo.clk_divr))
		return PTR_ERR(pc->sipo.clk_divr);

	pc->sipo.clk_dly = devm_platform_ioremap_resource(pdev, 15);
